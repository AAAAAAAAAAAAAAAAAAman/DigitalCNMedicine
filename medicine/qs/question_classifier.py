...
class QuestionClassifier:
    ...
    def build_wdtype_dict(self):
        wd_dict = {}
        category_map = {
            'disease': self.disease_wds,
            'department': self.department_wds,
            'check': self.check_wds,
            'drug': self.drug_wds,
            'food': self.food_wds,
            'symptom': self.symptom_wds,
            'producer': self.producer_wds
        }

        for word in self.region_words:
            categories = []
            for category, word_set in category_map.items():
                if word in word_set:
                    categories.append(category)
            wd_dict[word] = categories
